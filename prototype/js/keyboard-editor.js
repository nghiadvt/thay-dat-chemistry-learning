...
function renderProps() {
  ...
  if (key) {
    ...
    el.innerHTML = `
      <div class="kbe-props-section">
        <h3>Phím đã chọn</h3>
        <div class="kbe-field"><label>Text hiển thị</label>
          <input type="text" id="propKeyText" value="${esc(key.text)}"></div>
        <div class="kbe-field"><label>Giá trị output</label>
          <input type="text" id="propKeyValue" value="${esc(key.value)}" ${isDelete ? 'readonly' : ''}></div>
        <div class="kbe-field"><label>Width (units)</label>
          <input type="number" id="propKeyWidth" min="1" max="${MAX_UNITS}" value="${key.width}" ${isDelete || isSpecial ? 'readonly' : ''}></div>
        <div class="kbe-field"><label>Tooltip</label>
          <input type="text" id="propKeyTooltip" value="${esc(key.tooltip)}"></div>
      ...
      <div class="kbe-props-section">
        <h3>Style phím</h3>
        <div class="kbe-field"><label>Nền</label>
          <div class="kbe-field-row">
            <input type="color" id="propKeyBg" value="${toColor(key.background)}">
            <input type="text" id="propKeyBgHex" value="${key.background}" maxlength="7">
          </div></div>
        <div class="kbe-field"><label>Màu chữ</label>
          <div class="kbe-field-row">
            <input type="color" id="propKeyColor" value="${toColor(key.color)}">
            <input type="text" id="propKeyColorHex" value="${key.color}" maxlength="7">
          </div></div>
        <div class="kbe-field"><label>Viền</label>
          <div class="kbe-field-row">
            <input type="color" id="propKeyBorder" value="${toColor(key.border)}">
            <input type="text" id="propKeyBorderHex" value="${key.border}" maxlength="7">
          </div></div>
        <div class="kbe-field"><label>Bo góc (px)</label>
          <input type="number" id="propKeyRadius" min="0" max="20" value="${key.radius}"></div>
        ${isDelete ? '<p style="font-size:12px;color:#9ca3af;margin-top:8px">Phím Delete: width cố định 2 units, không thể xóa.</p>' : ''}
        ${!isDelete && !isSpecial ? `<div class="kbe-props-actions" style="margin-top:12px">
  ...
  if (row) {
    ...
    el.innerHTML = `
      <div class="kbe-props-section">
        <h3>Hàng đã chọn</h3>
        <div class="kbe-field"><label>Tên hàng</label>
          <input type="text" id="propRowName" value="${esc(row.name)}"></div>
        <div class="kbe-field"><label>Chiều cao</label>
          <div class="kbe-seg" id="propRowHeight">
            <button type="button" data-val="S" class="${row.height === 'S' ? 'active' : ''}">S</button>
            <button type="button" data-val="M" class="${row.height === 'M' ? 'active' : ''}">M</button>
            <button type="button" data-val="L" class="${row.height === 'L' ? 'active' : ''}">L</button>
          </div></div>
        <div class="kbe-field"><label>Padding (px)</label>
          <input type="number" id="propRowPad" min="0" max="20" value="${row.padding}"></div>
        <div class="kbe-field"><label>Spacing (px)</label>
          <input type="number" id="propRowSpace" min="0" max="20" value="${row.spacing}"></div>
        <div class="kbe-field"><label>Nền hàng</label>
          <div class="kbe-field-row">
            <input type="color" id="propRowBg" value="${toColor(row.background)}">
            <input type="text" id="propRowBgHex" value="${row.background}" maxlength="7">
          </div></div>
        <div class="kbe-field"><label>Viền hàng</label>
          <div class="kbe-field-row">
            <input type="color" id="propRowBorder" value="${toColor(row.border)}">
            <input type="text" id="propRowBorderHex" value="${row.border}" maxlength="7">
          </div></div>
        <div class="kbe-field"><label>Căn chỉnh</label>
        ...
        <p style="font-size:12px;color:${units > MAX_UNITS ? '#dc2626' : '#6b7280'};margin-top:4px">
          Units: ${units}/${MAX_UNITS}${units > MAX_UNITS ? ' — Vượt quá!' : ''}
        </p>
      </div>
      <div class="kbe-props-section">
        <h3>Hành động hàng</h3>
        <div class="kbe-props-actions">
          <button type="button" id="propDupRow">Nhân đôi hàng</button>
          <button type="button" id="propMoveUp" ${row.isSpaceRow ? 'disabled' : ''}>Di chuyển lên</button>
          <button type="button" id="propMoveDown" ${row.isSpaceRow ? 'disabled' : ''}>Di chuyển xuống</button>
          <button type="button" id="propAutoFill">Tự động lấp đầy hàng</button>
  ...
  el.innerHTML = `<div class="kbe-empty-props">Chọn hàng hoặc phím trên preview để chỉnh sửa thuộc tính.</div>${renderInfoCard()}`;
}
...
function syncDefaultOptions() {
  ...
  document.querySelectorAll('[data-opt="keySize"] button').forEach(b => {
    b.classList.toggle('active', b.dataset.val === d.keySize);
  });
  document.querySelectorAll('[data-opt="fontSize"] button').forEach(b => {
    b.classList.toggle('active', b.dataset.val === d.fontSize);
  });
  document.getElementById('kbeDefaultTextColor').value = toColor(d.textColor);
  document.getElementById('kbeDefaultTextHex').value = d.textColor;
  document.getElementById('kbeDefaultBgColor').value = toColor(d.background);
  document.getElementById('kbeDefaultBgHex').value = d.background;
  document.getElementById('kbeDefaultBorderColor').value = toColor(d.border);
  document.getElementById('kbeDefaultBorderHex').value = d.border;
}
...
function buildOverlayPhone(containerId) {
  ...
  container.innerHTML = `<div class="kbe-phone-wrap ${editor.device}">
    <div class="kbe-phone-frame">
      <div class="kbe-phone-screen">
...
function applyZoom() {
  ...
  if (wrap) wrap.style.transform = `scale(${editor.zoom})`;
}

function applyDevice() {
  const wrap = document.getElementById('kbePhoneWrap');
  if (wrap) {
    wrap.classList.toggle('tablet', editor.device === 'tablet');
  }
}
...
function initEditor() {
  ...
  document.getElementById('kbeExportBtn').addEventListener('click', exportJson);
  document.getElementById('kbeImportBtn').addEventListener('click', () => document.getElementById('kbeImportFile').click());
  document.getElementById('kbeImportFile').addEventListener('change', e => {
  ...
  document.querySelectorAll('[data-opt]').forEach(group => {
    group.querySelectorAll('button').forEach(btn => {
      btn.addEventListener('click', () => {
        const opt = group.dataset.opt;
        commit(() => { editor.data.defaults[opt === 'keySize' ? 'keySize' : 'fontSize'] = btn.dataset.val; });
      });
    });
  });
