...
const HTDGameAdapter = (function () {
    ...
    switch (payload.answer_type) {
      ...
      case 'structured':
        return {
          ...base,
          type: 'input',
          inputMode: payload.input_mode || 'balance',
          media: 'text',
  ...
  function buildSubmitPayload(question, uiState) {
    ...
    if (question.type === 'input') {
      if (question.inputMode === 'balance' || question.inputMode === 'blank_balance') {
        return uiState.inputValues || {};
      }
      const blank = uiState.inputValues?.blank || {};
      const first = Object.values(blank)[0] || '';
      if (question.inputMode === 'product' || question.inputMode === 'formula') {
        return first || '';
      }
      return { text: first };
    }
