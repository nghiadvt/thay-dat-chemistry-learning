...
# **5\. Thiết kế Bàn phím Hóa học — Mobile-first UX**

🎯  Nguyên tắc thiết kế: 1 ngón cái thao tác được toàn bộ. Không cần cuộn. Submit ngay sau khi nhập xong. Công thức hiển thị đúng như sách giáo khoa.

## **5.1 Layout bàn phím — 3 tab chuyển đổi**

**Tab 1 — Nguyên tố thường dùng (mặc định):** Các nguyên tố hay xuất hiện nhất trong chương trình phổ thông.

┌─────────────────────────────────────────────┐  
│  \[H\]   \[O\]   \[C\]   \[N\]   \[Na\]  \[K\]   \[⌫\] │  ← hàng 1: phổ biến nhất  
│  \[Ca\]  \[Fe\]  \[Cu\]  \[Al\]  \[Cl\]  \[S\]   \[Mg\] │  ← hàng 2  
│  \[P\]   \[Zn\]  \[Ag\]  \[Mn\]  \[Cr\]  \[Ba\]  \[Br\] │  ← hàng 3  
│ \[Tab:Nguyên tố\] \[Tab:Số\] \[Tab:Ký hiệu\]     │  ← tab bar  
│                    \[ SUBMIT \]               │  ← nút submit lớn  
└─────────────────────────────────────────────┘

**Tab 2 — Số & Hệ số:** Số to ở đầu (hệ số) và số nhỏ ở chỉ số dưới (subscript).

┌─────────────────────────────────────────────┐  
│  Input hiện tại: 2H₂O  ← preview live      │  
│                                             │  
│  Hệ số (số to, đứng trước):                 │  
│  \[1\]  \[2\]  \[3\]  \[4\]  \[5\]  \[6\]  \[⌫\]         │  
│                                             │  
│  Chỉ số dưới (subscript, trong công thức):  │  
│  \[₂\]  \[₃\]  \[₄\]  \[₅\]  \[₆\]  \[₇\]             │  
│  \[₈\]  \[₉\]  \[₁₀\] \[₁₂\]                       │  
│                    \[ SUBMIT \]               │  
└─────────────────────────────────────────────┘

**Tab 3 — Ký hiệu:** Dấu cộng, mũi tên, ngoặc, dấu nhân cho phương trình.
...
## **5.2 UX chi tiết — Input model**

Mỗi ký tự được phân loại rõ ràng để render đúng:

// Cấu trúc token — mỗi ký tự nhập là 1 token  
type TokenType \= 'coefficient' | 'element' | 'subscript' | 'symbol'  
   
interface Token {  
  type: TokenType  
  value: string       // raw: '2', 'H', '2' (subscript), '+'  
  display: string     // rendered: '2', 'H', '₂', '+'  
}  
   
// Ví dụ: người dùng nhập 2H₂O  
tokens \= \[  
  { type: 'coefficient', value: '2',  display: '2'  },  // to, đứng trước  
  { type: 'element',     value: 'H',  display: 'H'  },  
  { type: 'subscript',   value: '2',  display: '₂'  },  // nhỏ, thấp xuống  
  { type: 'element',     value: 'O',  display: 'O'  },  
\]  
   
// Serialize để gửi server  
serialize(tokens) → '2H2O'   // plain text, server normalize

## **5.3 Smart input — Tự động nhận diện context**

Bàn phím tự biết người dùng đang nhập gì để render đúng loại:

* **Vị trí đầu công thức hoặc sau '+' →** số tiếp theo là hệ số (to, bình thường).

* **Sau một nguyên tố →** số tiếp theo là chỉ số dưới (subscript, nhỏ và thấp).

* **Highlight nguyên tố vừa nhập →** 1 giây rồi mờ đi, giúp học sinh biết đã gõ đúng.

* **Nút ⌫ (backspace) →** xóa 1 token hoàn chỉnh, không phải 1 ký tự. Xóa 'Na' là xóa cả 2 chữ cái.

## **5.4 Visual design — CSS**

/\* Nút nguyên tố — Mobile touch target tối thiểu 44×44px \*/  
.key-element {  
  min-width: 44px; min-height: 44px;  
  background: \#1F4E79; color: white;  
  border-radius: 8px;  
  font-size: 16px; font-weight: 700;  
  box-shadow: 0 3px 0 \#0D3052;   /\* depth effect \*/  
  transition: transform 0.08s, box-shadow 0.08s;  
}  
.key-element:active {  
  transform: translateY(2px);  
  box-shadow: 0 1px 0 \#0D3052;   /\* nhấn xuống khi tap \*/  
}  
   
/\* Nút Submit — nổi bật, dễ bấm \*/  
.key-submit {  
...
/\* Preview công thức — font toán học \*/  
.formula-display {  
...
## **5.5 Khả năng mở rộng — Extensible keyboard registry**

Thiết kế theo dạng config-driven để thêm nguyên tố/ký hiệu không cần sửa code UI:

// keyboard-config.js — thêm nguyên tố chỉ cần edit file này  
export const KEYBOARD\_TABS \= \[  
  {  
    id: 'common',  
    label: 'Nguyên tố',  
    rows: \[  
...
// Để thêm tab Vô cơ nâng cao sau này:  
// chỉ cần push thêm 1 object vào KEYBOARD\_TABS
